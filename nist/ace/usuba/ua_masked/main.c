...
void ace(unsigned char text[40]) {
  uint32_t input[5][2][MASKING_ORDER];
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 2; j++) {
      uint32_t v = __builtin_bswap32(((uint32_t*)text)[i*2+j]);
      MASK_VAL(v,input[i][j]);
    }

  uint32_t output[5][2][MASKING_ORDER];
  ACE__(input,output);
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 2; j++) {
      uint32_t v;
      UNMASK_VAL(output[i][j],v);
      ((uint32_t*)text)[i*2+j] = __builtin_bswap32(v);
    }
}
...
void ace(unsigned char text[40]) {
  ...
  uint64_t text_bs_masked[5][64][MASKING_ORDER];
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 64; j++)
  ...
  for (int i = 0; i < 5; i++)
    for (int j = 0; j < 64; j++)
      UNMASK_VAL(output_bs_masked[i][j],output_bs[i][j]);
