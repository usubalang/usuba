...
int main() {
  ...
  DATATYPE *plain_ortho  = ALLOC(256);
  DATATYPE *cipher_ortho = ALLOC(256);
  uint64_t *plain_std = ALLOC(1024);


  while(fread(plain_std, 8, 1024, fh_in)) {

    for (int i = 0; i < 1024; i++)
      plain_std[i] = __builtin_bswap64(plain_std[i]);

    orthogonalize_256x256(plain_std, plain_ortho);
    ...
    //cipher_ortho = plain_ortho;
    
    unorthogonalize_256x256(cipher_ortho,plain_std);
    
    for (int i = 0; i < 1024; i++)
      plain_std[i] = __builtin_bswap64(plain_std[i]);

    fwrite(plain_std, 8, 1024, fh_out);
  }
