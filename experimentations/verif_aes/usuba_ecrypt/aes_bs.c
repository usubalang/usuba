...
void aes_encrypt(unsigned int counter[4],
                 DATATYPE key[11][8],
                 __m128i* out,
                 unsigned int len) {
  DATATYPE plain[8];
  ...
  for (i = 0; i < len; i++) {
    plain[i] = _mm_set_epi32(counter[0], counter[1], counter[2], counter[3]);
    counter[0]++;
  }

  aes_bs(plain,key,cipher);

  for (i = 0; i < len; i++) {
    _mm_store_si128(&out[i], cipher[i]);
  }
