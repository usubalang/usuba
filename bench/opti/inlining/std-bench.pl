...
my $out_w = 'des-no-inlining.c';
my $out_n = 'des-inlining.c';
...
system "./usubac -no-inlining $opts -o $self_dir/tmp/$out_n samples/usuba/des.ua";
...
\\begin{table}[ht!]
  \\begin{tabular}{|l K{1cm}|K{2cm}|K{2cm}|K{1.5cm}|}
    \\hline
    & \\textbf{CC} & \\textbf{w/o inlining} &\\textbf{w inlining} & \\textbf{speedup} \\\\
    \\hline\\hline
';
