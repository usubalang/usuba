...
This section presents the common parts of the benchmarks for section 3.2.
...
B<High-level description>
    ...
    The benchmarks runs interleaved and non-interleaved versions of
...
B<Specific details>
    ...
    scheduling)
    lines 85-89: run the benchmarks, stores the results.
...
B<Specific details>
    ...
    unrolling)
    lines 88-94: run the benchmarks, stores the results.
...
B<High-level description>
    ...
    This benchmark generates the file human.tex, which defines macros containing
...
B<sub-benchmarks>: 
    ...
    './data-do'. This _will_ take a few hours. We added in the repository
...
B<Collecting the results>. (this is actually the content of bench/ua-vs-human/run.pl).
    ...
    primitive. We manually counted the lines of code and hard-coded the
...
B<Technical details>
    ...
    it shouldn't be an issue: you'll just have a bunch of 0s instead
...
B<Technical details>
    ...
    This bench works quite like the benchmarks from section 3.2.
    It first compiles the Usuba files (if -g option was
    supplied) with each slicing (-B for bitslice, -V for vslice, -H
    for hslice). Then compiles the C benchmarks, and then run the
    benchmarks.
