...
#define load_input()                                                    \
  DATATYPE input[512],out_buff[512];                                    \
  int nb_blocks = 0;                                                    \
  for (; (signed_len > 0) && (nb_blocks < PARALLEL_FACTOR);             \
       signed_len -= BLOCK_SIZE*4, nb_blocks += 4) {                    \
    __m128i c[4];                                                       \
    for (int i = 0; i < 4; i++) c[i] = _mm_load_si128((__m128i*)counter); \
    input[nb_blocks/4] = _mm512_loadu_si512(c) ;                        \
  }
