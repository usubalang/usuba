...
#define rectangle(in,key,out) {                 \
    ...
    __m128i plain2[8]; memcpy(plain2,in+64,64);  \
    __m128i cipher[8];                          \
    __m128i cipher2[8];                          \
    nslice_8x64(plain);                         \
    nslice_8x64(plain2);                         \
    Rectangle__(plain,plain2,key,cipher,cipher2);              \
    nslice_8x64_undo(cipher);                   \
    nslice_8x64_undo(cipher2);                   \
    memcpy(out,cipher,64);                      \
    memcpy(out+64,cipher2,64);                      \
  }
...
#define rectangle(in,key,out) {                 \
    ...
    __m256i plain2[8]; memcpy(plain2,in+128,128);   \
    __m256i cipher[8];                          \
    __m256i cipher2[8];                          \
    nslice_16x64(plain);                        \
    nslice_16x64(plain2);                        \
    ...
    nslice_16x64_undo(cipher2);                  \
    memcpy(out,cipher,128);                     \
    memcpy(out+128,cipher2,128);                     \
  }
...
#define rectangle(in,key,out) {                 \
    ...
    __m512i plain2[8]; memcpy(plain2,in+256,256);     \
    __m512i cipher[8];                          \
    __m512i cipher2[8];                          \
    nslice_32x64(plain);                        \
    nslice_32x64(plain2);                        \
    ...
    nslice_32x64_undo(cipher2);                  \
    memcpy(out,cipher,256);                     \
    memcpy(out+256,cipher2,256);                     \
  }
...
void Rectangle__ (DATATYPE plain__[4],DATATYPE plain__2[4],DATATYPE key__[26][4],
                  DATATYPE cipher__[4],DATATYPE cipher__2[4]);
