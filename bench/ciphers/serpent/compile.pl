...
if ($gen) {
    ...
    for my $arch (qw( std sse avx )) {
        my $specific = $arch eq 'std' ? '-bits-per-reg 32' : '';
        system "./usubac $specific -interleave 10 -no-arr -arch $arch -no-sched -o bench/ciphers/serpent/$arch/$cipher.c samples/usuba/serpent.ua";
    }
    chdir "$FindBin::Bin";
    say " done.";
}
