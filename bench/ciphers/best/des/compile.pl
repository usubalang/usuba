...
my $cipher  = 'des';
my $arch    = 'avx';
my @CCs     = qw( gcc icc clang );

my @binaries;



if (0) { # Not regenerating DES for now
    if ($gen) {
        print "Compiling Usuba sources...";
        chdir "$FindBin::Bin/../../../..";
        system "./usubac -arch $arch -no-arr -sched-n 0 -o $FindBin::Bin/$arch/des.c samples/usuba/des.ua";
        chdir "$FindBin::Bin";
        say " done.";
    }
}
...
for my $CC (@CCs) {
    my $bin = "bin/$arch-$CC";
    my $cmd = "$CC $CFLAGS $HEADERS -D $arch main.c -o $bin";
