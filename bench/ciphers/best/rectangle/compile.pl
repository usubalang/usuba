...
my $cipher   = 'rectangle';
my $arch     = qw( avx );
my @slicings = qw( bitslice vslice hslice );
...
if ($gen) {
    ...
    system "./usubac $add_opt -lf -arch $arch -interleave 10 -o $FindBin::Bin/vslice/$arch.c samples/usuba/rectangle_vector.ua";
    chdir "$FindBin::Bin";
    say " done.";
}
