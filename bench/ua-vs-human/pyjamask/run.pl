...
if ($gen) {
    ...
    system "$usubac $ua_opts -o pyjamask-ua.c $source_file";
}
...
if ($compile) {
    ...
    for my $file (qw(pyjamask-ua pyjamask-ref)) {
...
if ($run) {
    ...
    for (1 .. $nb_run) {
        for my $bench (qw(pyjamask-ua pyjamask-ref)) {
            push @{$times{$bench}}, (`./$bin_dir/$bench` =~ s/ .*$//r)+0;
        }
    }

    my ($ua, $ua_stdev) = avg_stdev(@{$times{"pyjamask-ua"}});
    printf "Usuba: %.2f   +-%.2f\n", $ua, $ua_stdev;

    my ($ref, $ref_stdev) = avg_stdev(@{$times{"pyjamask-ref"}});
