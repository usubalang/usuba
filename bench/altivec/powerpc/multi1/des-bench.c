...
void single_des__ (uint64_t *buff_in, uint64_t* buff_out,
  ...
  /* Since the key get modified during the encryption,
     we need to copy it on the thread's stack.
     (otherwise, all threads will access it,
      thus needing to go find it in the L3 cache to
      propagate modifications (I think))*/
  DATATYPE key_ortho[BLOCK_SIZE];
  memcpy(key_ortho,key_in,KEY_SIZE*sizeof(DATATYPE));

  int id = omp_get_thread_num();

  for (int a = 0; a < NB_LOOPS; a++) /* with 3000, it should take ~160sec on 1 core */
...
int main() {
  ...
  // the buffers are twice bigger than the file.
  // (so we can go a bit further at the end)
  uint64_t* buff_in  = ALLOC(size*2);
  uint64_t* buff_out = ALLOC(size*2);
  ...
  fclose(fh_in);
  // Filling the 2nd half of the buffer
  memcpy(&buff_in[size/8],buff_in,size);

  for (int i = 1; i <= 32; i += i < 4 ? 1 : i) {
    // NOTE: for benching, we make sure the size of the buffer
    // is a multiple on the number of cores.
    // It means we assume that the situation is ideal.
    // A small overhead would apply if the size wasn't a multiple
    // of the number of cores.
    long loc_size = size/8;
    while (loc_size/BLOCK_SIZE % i != 0) loc_size++;
