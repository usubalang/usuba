...
void des__ (DATATYPE plaintext__[64],DATATYPE key__[64], DATATYPE ciphered__[64]);


/* Doesn't XOR, not isn't CTR, but works as glue */
...
/* unsigned char key[] = { 0xF1, 0xDF, 0xBC, 0x9B, 0x79, 0x57, 0x34, 0x13 }; */
  ...
  while(inlen > 0) {
    ...
    uint64_t* plain_std = (uint64_t*)out;
    memcpy(plain_std,in,CHUNK_SIZE*8);
