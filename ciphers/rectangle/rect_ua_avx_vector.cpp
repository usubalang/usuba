...
#include <x86intrin.h>
/* main function */
void RectangleAVX__ (/*inputs*/ unsigned char* plain,unsigned char* key__, /*outputs*/ unsigned char* cipher) {
...
#define XOR_KEY(a,b,n) {                                        \
    DATATYPE tmp = _mm256_set1_epi16(((unsigned short*)key__)[n]); \
    a = XOR(b,tmp);                                             \
  }
  ...
  for (int i = 0; i <= 24; i++) {
    ...
    XOR_KEY(_tmp1_[0],tmp__[0],i*4+0);
    XOR_KEY(_tmp1_[1],tmp__[1],i*4+1);
    XOR_KEY(_tmp1_[2],tmp__[2],i*4+2);
    XOR_KEY(_tmp1_[3],tmp__[3],i*4+3);
  ...
  XOR_KEY(cipher__[0],tmp__[0],25*4+0);
  XOR_KEY(cipher__[1],tmp__[1],25*4+1);
  XOR_KEY(cipher__[2],tmp__[2],25*4+2);
  XOR_KEY(cipher__[3],tmp__[3],25*4+3);
