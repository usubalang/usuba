...
#define rectangle(in,key,out) RectangleGP__(in,in+8,key,out,out+8)
...
#define rectangle(in,key,out) {                             \
  ...
  RectangleSSE__(in,in+(PARALLEL_FACTOR * BLOCK_SIZE)/2,    \
                 (unsigned char*)key,                       \
                 out,out+(PARALLEL_FACTOR * BLOCK_SIZE)/2); \
...
#define rectangle(in,key,out) {                             \
    ...
    TRANSPOSE4_IN(plain[0],plain[1],plain[2],plain[3]);     \
    TRANSPOSE4_IN(plain[4],plain[5],plain[6],plain[7]);     \
  RectangleAVX__(in,in+(PARALLEL_FACTOR * BLOCK_SIZE)/2,    \
                 (unsigned char*)key,                       \
                 out,out+(PARALLEL_FACTOR * BLOCK_SIZE)/2); \
  TRANSPOSE4_OUT(cipher[0],cipher[1],cipher[2],cipher[3]);  \
  TRANSPOSE4_OUT(cipher[4],cipher[5],cipher[6],cipher[7]);  \
