...
let expand_in_node (env_fun:(ident,def) Hashtbl.t) (f:def) (vd:var_d)
  ...
  let new_p =
    List.map
      (fun i -> let id' = fresh_suffix vd.vd_id (sprintf "%d'" i) in
                ...
                make_var_d id' new_typ vd.vd_inplace vd.vd_opts vd.vd_orig)
  ...
  let new_node =
    if is_p_in then
      { f with p_in = replace f.p_in (make_var_d vd.vd_id vd.vd_typ vd.vd_inplace
                                                 vd.vd_opts vd.vd_orig) new_p;
               node = Single(get_vars f.node,body); }
    else
      { f with p_out = replace f.p_out (make_var_d vd.vd_id vd.vd_typ vd.vd_inplace
                                                   vd.vd_opts vd.vd_orig) new_p;
