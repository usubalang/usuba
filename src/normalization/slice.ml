...
(*     let l' = List.map (fun vd -> let typ, b = build_type n m vd.vd_typ in *)
...
(*     let in_size = match (List.hd def.p_in).vd_typ with *)
(*       | Int(t_in,t_out) -> t_in * t_out *)
(*       | _ -> assert false in *)
(*     let p_in  = { (List.hd def.p_in)  with vd_typ = Int(1,n*m) } in *)
(*     let p_out = { (List.hd def.p_out) with vd_typ = Int(1,n*m) } in *)
...
(*            [ *)
(*              Eqn([Slice(Var p_out.vd_id,slice)], *)
(*                  Fun(new_id,[ExpVar(Slice(Var p_in.vd_id,slice))]), *)
(*                  false) *)
(*            ], *)
(*            []) *)
(*     ] in *)
...
(*                      p_in  = List.map (fun vd -> { vd with vd_typ = Int(1,n*m)}) def.p_in; *)
(*                      p_out = List.map (fun vd -> { vd with vd_typ = Int(1,n*m)}) def.p_out; *)
(*                      opt   = []; *)
(*                      node  = Single([], body) } in *)
(*     [ table; new_node ] *)
...
(*     match get_base_type (List.hd ((last prog.nodes).p_in)).vd_typ with *)
(*     | Int(n,m) -> n,m *)
(*     | _ -> assert false *)
