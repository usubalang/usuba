...
module Rewriter (Aux : SPECIFIC_REWRITER ) = struct
                        ...
                        if Aux.keep_print then
                          (match l with
                           | (Var id)::[] ->
                              ...
                              | None -> raise Invalid_operator_call)
                        else Nop
                      else
  ...
  let rewrite_prog (p: prog) : prog =
    ...
    let entry_point = Aux.gen_entry_point (List.nth p (List.length p -1)) in
    entry := entry_point ;
