...
let rec expand_with_iterators (its:(ident*arith_expr*arith_expr) list)
  ...
  let rec aux (remaining:(ident*arith_expr*arith_expr) list)
    ...
    | (idx,ei,ef) :: tl ->
       if Hashtbl.mem used_iterators idx then
         ...
         flat_map (fun i -> Hashtbl.add env_it idx i;
                            let r = aux tl in
                            Hashtbl.remove env_it idx;
                            r) (gen_list_bounds ei ef)
...
let rec get_used_vars_var (its:(ident*arith_expr*arith_expr) list)
  ...
  | Index(v',ae) ->
     ...
     (match simpl_arith_ne ae with
      ...
      | ae' -> (* The index contains one or more variables *)
...
let rec get_used_vars_expr (its:(ident*arith_expr*arith_expr) list)
  ...
  | Log(_,x,y) | Arith(_,x,y) -> (get_used_vars_expr its env_var x)
                                 @ (get_used_vars_expr its env_var y)
...
let rec schedule_preds (its:(ident*arith_expr*arith_expr) list)
                       (env_var:(ident, typ) Hashtbl.t)
                       (ready:(var,deq) Hashtbl.t)
                       (computed:(deq,bool) Hashtbl.t)
                       (result:deq list ref)
                       (deq:deq) : unit =
  List.iter (fun v ->
             match Hashtbl.find_opt ready v with
             | Some deq' -> if Hashtbl.mem computed deq' then ()
                            else (
...
let add_to_ready (its:(ident*arith_expr*arith_expr) list)
  ...
  List.iter (fun v -> Hashtbl.add ready v deq)
...
let rec schedule_deqs (its:(ident*arith_expr*arith_expr) list)
                      ...
                      (deqs:deq list)
    : deq list =
  let result   = ref [] in (* |deqs| but correctly scheduled; to be reversed at the end. *)
  let computed = Hashtbl.create 10 in (* Variables already computed *)
  let ready    = Hashtbl.create 10 in (* Equations ready to be scheduled *)
  ...
  List.iter (fun deq ->
             ...
             | Eqn(_,Fun _,_) ->
                ...
                result := deq :: !result;
             | Eqn(lhs,_,_) ->
                add_to_ready its env_var ready lhs deq
             | Loop(i,ei,ef,dl,opts) ->
                Hashtbl.add computed deq true;
                let its = (i, ei, ef) :: its in
                schedule_preds its env_var ready computed result deq;
                result := { deq with
                            content = Loop(i,ei,ef, schedule_deqs its env_var dl,
                                           opts) } :: !result
            ) deqs;
  ...
  List.iter (fun deq -> if Hashtbl.mem computed deq then ()
                        else result := deq :: !result) deqs;

  List.rev !result

(* Returns true if |deqs| contains a function call. *)
let rec deqs_contain_funcall (deqs:deq list) : bool =
  List.exists (fun deqs ->
               match deqs.content with
               | Eqn(_,Fun _,_) -> true
               | Eqn _ -> false
               | Loop(_,_,_,dl,_) -> deqs_contain_funcall dl) deqs
