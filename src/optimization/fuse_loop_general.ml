...
let rec instanciate_var (env_it:(ident,int) Hashtbl.t) (v:var) : var =
  match v with
  | Var _ -> v
  | Index(v',idx) -> Index(instanciate_var env_it v',
                           Const_e (eval_arith env_it idx))
  | _ -> assert false
...
let push_it_env (env_it:it_env_t) (id:ident)
                (ei:arith_expr) (ef:arith_expr) : unit =
  env_it := { id=id; ei=ei; ef=ef } :: !env_it
(* Removes the first iterator from the environment *)
let pop_it_env (env_it:it_env_t) : unit =
  env_it := List.tl !env_it
...
let iter_env_it (env_it:it_env_t) (f:((ident,int) Hashtbl.t) -> unit) : unit =
  ...
  let rec aux (env_it:iterator list) : unit =
    ...
    | it :: its ->
       ...
       List.iter (fun i_val -> Hashtbl.replace concrete_env it.id i_val;
                               aux its;
...
let is_ready_expr (env_var:(ident,typ) Hashtbl.t)
                  (env_it:it_env_t)
  ...
  iter_env_it env_it
    (fun env_it ->
     List.iter (fun v ->
                List.iter
                  (fun v -> if not (Hashtbl.mem env_ready (instanciate_var env_it v)) then
                              is_ready := false)
                  (expand_var env_var ~env_it:env_it v))
               used_vars);
  !is_ready

let rec is_ready_deq (env_var:(ident,typ) Hashtbl.t)
                     (env_it:it_env_t)
                     (env_ready:(var,bool) Hashtbl.t)
                     (deq:deq) : bool =
  ...
  (match deq with
   ...
   | Loop(i,ei,ef,dl,_) ->
      ...
      is_ready := !is_ready &&
                    (List.for_all (is_ready_deq  env_var env_it env_ready) dl);
      pop_it_env env_it);
  !is_ready

let is_ready_loop (env_var:(ident,typ) Hashtbl.t)
                  (env_it:it_env_t)
                  (env_ready:(var,bool) Hashtbl.t)
                  (loop:loop) : bool =
  push_it_env env_it loop.id loop.ei loop.ef;
  let is_ready =
    List.for_all (is_ready_deq env_var env_it env_ready) loop.dl in
  pop_it_env env_it;
  is_ready


let is_mergeable (env_var:(ident,typ) Hashtbl.t)
                 (env_it:it_env_t)
                 (env_ready:(var,bool) Hashtbl.t)
                 (loop1:loop) (loop2:loop) : bool =
  let res = ref true in
        ...
        | Eqn(lhs,e,_) ->
           if check_ready then
             List.iter (fun v -> if not (Hashtbl.mem env_ready
                                          (instanciate_var env_it v)) then
                                   res := false)
                       (flat_map (expand_var env_var ~env_it:env_it)
                                 (get_used_vars e));
           List.iter (fun v -> Hashtbl.replace env_ready (instanciate_var env_it v) true)
                     (flat_map (expand_var env_var ~env_it:env_it) lhs)
        | Loop(i,ei,ef,dl,_) ->
           let ei = eval_arith env_it ei in
           let ef = eval_arith env_it ef in
           List.iter (fun i_val -> Hashtbl.add env_it i i_val;
                                   iter_loop env_it check_ready dl;
                                   Hashtbl.remove env_it i) (gen_list_bounds ei ef)
      ) deqs in
...
let rec update_env_ready (env_var:(ident,typ) Hashtbl.t)
                         (env_it:it_env_t)
                         (env_ready:(var,bool) Hashtbl.t)
                         (deq:deq) : unit =
  match deq with
  | Eqn(lhs,_,_) ->
     iter_env_it env_it
       (fun env_it ->
        List.iter (fun v -> List.iter (fun v -> Hashtbl.replace env_ready (instanciate_var env_it v) true)
                                      (expand_var env_var ~env_it:env_it v)) lhs)
  | Loop(i,ei,ef,dl,_) ->
     push_it_env env_it i ei ef;
...
let rec partition_deqs (env_var:(ident,typ) Hashtbl.t)
                       (env_it:it_env_t)
                       (env_ready:(var,bool) Hashtbl.t)
                       (outer:loop)
                       (nexts:deq list) :
                    ...
                    if (* For now, ignoring any loop that doesn't use
                      ...
                      ((outer.id = loop.id) && (outer.ei = loop.ei) && (outer.ef = loop.ef)) &&
                        ...
                        (* (is_ready_loop env_var env_it env_ready loop) && *)
                         (is_mergeable env_var env_it (Hashtbl.copy env_ready) outer loop) then
                      (push_it_env env_it i' ei' ef';
...
let rec fuse_loops_deqs (env_var:(ident,typ) Hashtbl.t)
                        (env_it:it_env_t)
                        (env_ready:(var,bool) Hashtbl.t)
  ...
  | hd :: nexts ->
     ...
     | Loop(i,ei,ef,dl,opts) ->
        ...
        pop_it_env env_it;
        let loop = { (loop_rec_of_sum hd) with dl=dl } in
