...
let rec add_right env_var live e =
  ...
  | ExpVar v ->
      List.iter
        (fun v -> Hashtbl.replace live (get_var_base v) true)
...
let remove_left env_var live lhs =
  List.iter
    (fun v ->
      List.iter
        (fun v -> Hashtbl.remove live (get_var_base v))
        (expand_var env_var v))
    lhs
  ...
  List.iter
    (fun x ->
      List.iter
        ...
        (expand_var env_var (Var x.vd_id)))
    p_out;
...
let live_def (def : def) : int =
  ...
  | Single (vars, body) -> (
      ...
      try live_deqs env_var def.p_out body with Not_implemented _ -> -1)
  | _ -> -1

let get_live_var (prog : prog) : int = max_l (List.map live_def prog.nodes)
