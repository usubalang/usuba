...
let rec add_right env_var live e =
  ...
  | ExpVar v ->
      List.iter
        (fun v -> VarHashtbl.replace live (Utils.get_var_base v) true)
...
let remove_left env_var live lhs =
  List.iter
    (fun v ->
      List.iter
        (fun v -> VarHashtbl.remove live (Utils.get_var_base v))
        (Utils.expand_var env_var v))
    lhs

let live_deq env_var live (deq : deq) : int =
  let nb_live = VarHashtbl.length live in
...
let live_deqs env_var (p_out : p) (deqs : deq list) : int =
  let live = VarHashtbl.create 50 in
  List.iter
    (fun x ->
      List.iter
        (fun x -> VarHashtbl.add live x true)
