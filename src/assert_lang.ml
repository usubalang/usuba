...
module Usuba0 = struct
  let is_usuba0 (prog : prog) : bool = true
end

module Usuba_norm = struct
  let check_var (var : var) : bool = match var with Var _ -> true | _ -> false

  let check_pat_e (p : var list) (e : expr) : bool =
    match e with Tuple l -> List.length l = List.length p | _ -> true
  ...
  let check_p (p : p) : bool =
    List.for_all (function (_, Array _), _ -> false | _ -> true) p
  ...
  let check_def (def : def) : bool =
    ...
    | Single (vars, body) ->
        ...
        && List.for_all
             (function
               | Eqn (p, e) ->
                   List.for_all check_var p && check_expr e && check_pat_e p e
               | _ -> false)
             body
    | _ -> false

  let is_usuba_normalized (prog : prog) : bool =
    Usuba0.is_usuba0 prog && List.for_all check_def prog.nodes
end
