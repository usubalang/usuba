...
let rec gen_list_bounds (i : int) (f : int) : int list =
  if i < f then i :: gen_list_bounds (i + 1) f
  else if i > f then i :: gen_list_bounds (i - 1) f
  else [ f ]
...
let int_to_boollist (n : int) (size : int) : bool list =
  let rec aux i l =
    if i = 0 then List.rev l else aux (i - 1) (((n lsr (i - 1)) land 1 = 1) :: l)
  in
  aux size []
...
let rec first_n l n =
  ...
  | 0 -> []
  | n ->
      if n > 0 then List.hd l :: first_n (List.tl l) (n - 1)
      else (
...
let rec gen_minus_one (s : int) =
  if s <= 0 then 0 else (1 lsl (s - 1)) lor gen_minus_one (s - 1)

(* Like fold_left, but with an index as well.
   Default start index is 0.
   If |start| is provided, then starts at this index. *)
let fold_left_i ?(start : int = 0) (f : int -> 'a -> 'b -> 'a) (init : 'a)
    (l : 'b list) : 'a =
  let i = ref start in
  List.fold_left
    (fun a b ->
...
(* Like fold_right, but with an index as well.
   Default start index is 0.
   If |start| is provided, then starts at this index. *)
let fold_right_i ?(start : int = 0) (f : int -> 'a -> 'b -> 'b) (l : 'a list)
    (init : 'b) : 'b =
  let i = ref start in
  List.fold_right
    (fun a b ->
...
let list_to_tuple1 l = match l with [ x ] -> x | _ -> assert false
let list_to_tuple2 l = match l with [ x; y ] -> (x, y) | _ -> assert false
