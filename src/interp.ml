...
module Usuba0 = struct
  let rec interp_expr env_fun (env_params : typ Ident.Hashtbl.t)
      (env_var : bool VarHashtbl.t) (e : expr) : bool list =
    let interp_expr_rec = interp_expr env_fun env_params env_var in
    match e with
    | Const (0, _) -> [ false ]
    | Const (1, _) -> [ true ]
    | ExpVar v -> [ VarHashtbl.find env_var v ]
  ...
  and interp_node env_fun (f : def) (l : bool list) : bool list =
    ...
    let interp_asgn env_fun (env_params : typ Ident.Hashtbl.t)
        (env_var : bool VarHashtbl.t) (vars : var list) (expr : expr) : unit =
      let res = interp_expr env_fun env_params env_var expr in
      List.iter2
        (fun x y -> VarHashtbl.replace env_var x y)
        (Basic_utils.flat_map (Utils.expand_var env_params) vars)
        res
    in
    (* creating a lexical environement *)
    let env_var : bool VarHashtbl.t = VarHashtbl.create 100 in
    (* adding the inputs in the environment *)
    let env_params = Utils.build_env_var f.p_in f.p_out (get_vars f) in
    List.iter2
      (fun v y -> VarHashtbl.replace env_var v y)
    ...
    List.map (VarHashtbl.find env_var)
      (Basic_utils.flat_map
         (Utils.expand_var env_params)
  ...
  let interp_prog (prog : prog) (input : bool list) : bool list =
    ...
    List.iter (fun f -> Ident.Hashtbl.replace env_fun f.id f) prog.nodes;
