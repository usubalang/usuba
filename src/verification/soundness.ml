...
let compare_tables (orig : def) (norm : def) =
  ...
  for i = 0 to pow 2 out_size - 1 do
    ...
    if out_orig <> out_norm then
      raise
        (Unsound
           (Printf.sprintf "%s: %d => expect:%d -- got:%d" orig.id.name i
              (boollist_to_int out_orig) (boollist_to_int out_norm)))
  done
    ...
    (fun x ->
      ...
      | Table _ -> (
          try
            let normed =
              List.find
                (fun def -> contains def.id.name x.id.name)
                normalized.nodes
            in
            compare_tables x normed
          with Not_found -> ())
      | _ -> ())
    (runner#run_module ~conf:default_conf Expand_multiples.as_pass orig).nodes
