...
let rec expr_to_bdd env man (e:expr) =
  ...
  | Log(op,e1,e2) -> let e1 = expr_to_bdd env man e1 in
  ...
  | _ -> Printf.printf "Can't convert '%s' to BDD.\n" (Usuba_print.expr_to_str e);
         assert false
                       
let deqs_to_bdd (node1:def) (deqs1:deq list) (node2:def) (deqs2:deq list) =
  let man = MLBDD.init () in
  
  let env1 = make_env () in
  let env2 = make_env () in

  List.iteri (fun i ((id,_),_) -> env_add env1 id (MLBDD.ithvar man i)) node1.p_in;
  List.iteri (fun i ((id,_),_) -> env_add env2 id (MLBDD.ithvar man i)) node2.p_in;
