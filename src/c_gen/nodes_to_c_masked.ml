...
let single_to_c (def:def) (array:bool) (vars:p)
  ...
  List.iter (fun vd -> if is_const vd then Hashtbl.add env_const vd.vd_id true) def.p_in;
  List.iter (fun vd -> if is_const vd then Hashtbl.add env_const vd.vd_id true) vars;
