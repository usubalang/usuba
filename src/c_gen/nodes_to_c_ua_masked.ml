...
let log_op_to_c_masked = function
  | Masked And -> "MASKED_AND"
  | Masked Or -> "MASKED_OR"
  | _ -> assert false
...
let rec expr_to_c (lift_env : (var, int) Hashtbl.t)
  ...
  | _ ->
      raise
        (Error
           (Printf.sprintf "Nested expressions not supported: %s.\n"
              (Usuba_print.expr_to_str e)))

let expr_to_c_ret (lift_env : (var, int) Hashtbl.t)
    (env : (string, string) Hashtbl.t) (env_var : (ident, typ) Hashtbl.t)
    (retvar : var) (e : expr) : string =
  let ret = var_to_c lift_env env retvar in
  match e with
  | Const (n, Some typ) -> sprintf "%s = %s" ret (const_to_c (get_type_m typ) n)
  | ExpVar v -> sprintf "%s = %s" ret (var_to_c lift_env env v)
  | Not e -> sprintf "%s = NOT(%s)" ret (expr_to_c lift_env env env_var e)
  | Log (op, x, y) -> (
      match op with
      | And | Or | Xor ->
          sprintf "%s = %s(%s,%s)" ret (log_op_to_c op)
            (expr_to_c lift_env env env_var x)
            (expr_to_c lift_env env env_var y)
      | Masked And | Masked Or ->
          sprintf "%s(%s,%s,%s)" (log_op_to_c_masked op) ret
            (expr_to_c lift_env env env_var x)
            (expr_to_c lift_env env env_var y)
      | _ -> assert false)
  | Arith (op, x, y) -> (
      match get_normed_expr_m env_var e with
      | Mint mval ->
          sprintf "%s(%s,%s,%s,%d)" (arith_op_to_c_generic op) ret
            (expr_to_c lift_env env env_var x)
            (expr_to_c lift_env env env_var y)
            mval
      | Mnat -> sprintf "%s = %s" ret (expr_to_c lift_env env env_var e)
      | _ -> assert false)
  | Shift (op, e, ae) ->
      sprintf "%s = %s(%s,%s,%d)" ret (shift_op_to_c op)
...
let fun_call_to_c (lift_env : (var, int) Hashtbl.t)
  ...
  sprintf "%s%s(%s,%s);" tabs (rename f.name)
    (join "," (List.map (expr_to_c lift_env env env_var) args))
    ...
    (List.map
       (fun deq ->
         ...
         | Eqn ([ vl ], Fun (f, [ vr ]), _) when f.name = "refresh" ->
             ...
             sprintf "%sREFRESH(%s,%s);" tabs (var_to_c lift_env env vl)
         ...
         | Eqn ([ v ], e, _) -> (
             ...
             | Mnat ->
                 sprintf "%s%s = %s;" tabs (var_to_c lift_env env v)
                   (expr_to_c lift_env env env_var e)
             | _ ->
                 sprintf "%s%s;" tabs (expr_to_c_ret lift_env env env_var v e))
         | Loop (i, ei, ef, l, _) ->
...
let var_decl_to_c conf (vd : var_d) _ : string =
...
let single_to_c (def : def) (_ : bool) (vars : p) (body : deq list)
    (conf : config) : string =
