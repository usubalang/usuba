...
let expr_to_c_ret (lift_env : (var, int) Hashtbl.t)
  ...
  match e with
  | Const (n, Some typ) ->
      Format.sprintf "%s = %s" ret (const_to_c (Utils.get_type_m typ) n)
  | ExpVar v -> Format.sprintf "%s = %s" ret (var_to_c lift_env env v)
  | Not e ->
      Format.sprintf "%s = NOT(%s)" ret (expr_to_c lift_env env env_var e)
  | Log (op, x, y) -> (
      match op with
      | And | Or | Xor ->
          Format.sprintf "%s = %s(%s,%s)" ret (log_op_to_c op)
            (expr_to_c lift_env env env_var x)
            (expr_to_c lift_env env env_var y)
      | Masked And | Masked Or ->
          Format.sprintf "%s(%s,%s,%s)" (log_op_to_c_masked op) ret
            (expr_to_c lift_env env env_var x)
            (expr_to_c lift_env env env_var y)
      | _ -> assert false)
  | Arith (op, x, y) -> (
      ...
      | _ -> assert false)
  | Shift (op, e, ae) ->
      Format.sprintf "%s = %s(%s,%s,%d)" ret (shift_op_to_c op)
...
let var_decl_to_c conf (vd : var_d) _ : string =
  ...
  | _ -> Format.sprintf "%s %s" vtype vname
