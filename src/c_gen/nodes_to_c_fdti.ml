...
let rec expr_to_c (lift_env : (var, int) Hashtbl.t) (conf : config)
    (env : (string, string) Hashtbl.t) (env_var : (ident, typ) Hashtbl.t)
    (e : expr) : string =
  match e with
  | Const (n, _) -> (
...
(* TODO: this 64 and 32 shouldn't be hardcoded *)
let expr_to_c_ret (lift_env : (var, int) Hashtbl.t) (conf : config)
    (env : (string, string) Hashtbl.t) (env_var : (ident, typ) Hashtbl.t)
  ...
  | Log (op, x, y) ->
      sprintf "%s(%s,%s,%s)" (log_op_to_c op) ret
        (expr_to_c lift_env conf env env_var x)
        (expr_to_c lift_env conf env env_var y)
  | Arith (op, x, y) ->
      (*Printf.fprintf stderr "Hardcoded arith op size\n";*)
      sprintf "%s(%s,%s,%s,%d)" (arith_op_to_c_generic op) ret
        ...
        32
  | Shuffle (v, l) ->
      sprintf "%s = PERMUT_%d(%s,%s)" ret (List.length l)
        (var_to_c lift_env env v)
        (join "," (List.map string_of_int l))
  | Shift (op, e, ae) ->
      (*Printf.fprintf stderr "Hardcoded rotation size\n";*)
      sprintf "%s(%s,%s,%s,%d)" (shift_op_to_c op) ret
        (expr_to_c lift_env conf env env_var e)
        (aexpr_to_c ae)
        (get_expr_reg_size env_var e)
  | Fun (f, [ v ]) when f.name = "rand" ->
...
let inputs_to_arr (def : def) : (string, string) Hashtbl.t =
  ...
  let aux (marker : string) vd =
    ...
    | _ ->
        ...
        raise (Not_implemented "Arrays as input")
  in
...
let var_decl_to_c conf (vd : var_d) (out : bool) : string =
  ...
  let rec aux (id : ident) (typ : typ) start =
    ...
    | Array (typ, size) -> aux id typ (sprintf "[%s]" (aexpr_to_c size))
  in
