...
static inline uint32_t rol32(uint32_t x, uint32_t s)
{
	return ((x << s) | (x >> (32 - s)));
}

#define ror32(x, s) rol32(x, 32 - (s))
...
void serpent_init(struct serpent_ctx *ctx, const uint8_t *key, unsigned int keylen)
{
	u32 *k = ctx->expkey;
	u8  *k8 = (u8 *)k;
	u32 r0, r1, r2, r3, r4;
