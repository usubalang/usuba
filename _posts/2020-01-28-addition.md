...
We compiled our C codes using Clang 7.0.0. We tried GCC 8.3.0 as well,
but it generates sub-optimal codes, containing more instructions than
necessary. Finally, we ran the benchmarks on a Intel Skylake i5-6500.
...
output carry is never used and doesn't need to be computed. The total
numbers of operations of the _n_-bit adder is therefore _n*5-6_.
Those operations are solely bitwise instructions, which can be
executed on ports 0, 1 and 5 on SSE registers (and on port 6 as well
on GP registers). This adder is therefore bound to run in at least
_n*5/3_ cycles.

In isolation, a full adder contains inner dependencies, preventing it
from executing in less than 3 cycles. However, in an _n_-bit adder,
out-of-order execution enables multiple full adders to execute
simultaneously, allowing a saturation of CPU ports 0, 1 and 5 despite
the dependencies within each full adder.
...
This slow down is explained by 2 factors:
 ...
 - Spilling: a bitsliced _n_-bit adder has _n*2_ inputs, _n_ outputs,
   ...
   variables are short lived, the register pressure remains high:
   there are only 16 SSE registers available and 15 GP registers
 ...
 - Desctructive 2-operand instructions. x86 bitwise instructions take
   ...
   enough to slightly slow down the bitslice adders. AVX introduced
   non-destructive 3-operand instructions. Using those instead of the
   SSE ones, the 8-bit adder executes at its optimal speed (it does
   not contain spilling). The other adders remain affected from
   spilling, but improve accross the board.
...
Using bitslicing to do an addition is 2 to 6 times slower than using
native `add` instructions. We did not benchmark the multiplication,
but since a multiplier requires at least _n*n_ instructions (whereas
an adder only requires _n*5_), we can expect it to behave even more
poorly compared to native instructions.
