...
comsumtions [18], or designing hardware gates whose power consumption
is independent of their inputs [10,19]. The most popular
countermeasures against SPA and DPA are however based on _secret
sharing_ [2,3], since they offer provable security.
...
Tymen's argument was only experimental).
...
<!-- security / models -->
Most of the masking schemes discussed so far have been shown to be
vulnerable _higher order DPA_ (HO-DPA) [1,20,33,34], where an attacker
combines multiple sources of leakage to reconstruct the secrets. This
called for higher order masking (_i.e._ masking with more than more
than one share) [7,41,43,44,45,46]. In fact, some schemes were even
vulnerable to first-order DPA [14,15], which called for provably
...
leakage model to the probing model: security in the probing model
implies security in the noisy leakge model.
