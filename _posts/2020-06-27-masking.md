...
which leak together. We integrated tightPROVE<sup>+</sup> in Usuba
[68] (and called the result _Tornado_) to generate provably
_t_-probing secure masked implementations in either the bit-probing or
the register-probing model. In the following post, we will describe
how the integration of tightPROVE<sup>+</sup> in Usubac was done, and
how Usubac then automatically masks ciphers.
