...
- (2014) SMT-based verification of software countermeasures against
  side-channel attacks. Checks for perfect masking (ie, better than
  Sleuth, because sleuth only checks wether values depends not _only_
  on secrets, whereas this works checks wether statistically
  independent on the inputs). Uses SMT solver (Yices) in LLVM. Tool
  name: SC Sniffer.
  (+ "Formal Verification of Software Countermeasures against
     Side-Channel Attacks")

- (2014) Synthesis of masking countermeasures against side channel
  attacks: based on "SMT-based verification of software
  countermeasures against side-channel attacks" (above), but this time
  synthesize the countermeasures. Tool name: SC Masker. Uses
  _inductive synthesis_: iteratively add countermeasures until the SMT
  solver proves the program to be perfectly masked. (for perf: masks
  the program region by region (may be less optimal than a monolithic
  approach, but faster to compute)
