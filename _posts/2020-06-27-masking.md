...
probability distribution of _x_, rather than revealing _x_ altogether
(and it thus supposed to be more realistic than the probing
model). Duc _et al._ [40] then showed a reduction of the noisy leakage
model to the probing model of Ishai _et al._ [7]: security in the
probing model implies security in the noisy leakage model.
...
which is tight in the sense that the number of refreshes inserted by
tightPROVE is minimal.
