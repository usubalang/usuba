...
implementations were only using two shares [6,4,15,5], which is
...
implementations [1,20,33,34]. When the masking order is greater than
one, we talk about _higher-order_ masking [7,41,43,44,45,46].

Each operation of the original cipher is replaced by a masked
_gadget_, which is a functionally equivalent software implementation
of the operation, except that it operates on shared variables, and
preserves the randomness of the masking.
