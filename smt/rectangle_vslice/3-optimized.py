...
def orig_Rectangle_(plain_,key_):
  ...
  for i_ in range(0,24+1):
    ...
    tmp_[(i_ + 1)][0] = _tmp2_[0]
    tmp_[(i_ + 1)][1] = RotateLeft(_tmp2_[1],1)
    tmp_[(i_ + 1)][2] = RotateLeft(_tmp2_[2],12)
    tmp_[(i_ + 1)][3] = RotateLeft(_tmp2_[3],13)
...
def dest_Rectangle_(plain_,key_):
  ...
  tmp_[3] = plain_[3]
  for i_ in range(0,24+1):
    _tmp1_[0] = tmp_[0] ^ key_[i_][0]
    _tmp1_[1] = tmp_[1] ^ key_[i_][1]
    _tmp1_[2] = tmp_[2] ^ key_[i_][2]
    _tmp1_[3] = tmp_[3] ^ key_[i_][3]
    (_tmp2_[0],_tmp2_[1],_tmp2_[2],_tmp2_[3]) = dest_SubColumn_V16(_tmp1_[0],_tmp1_[1],_tmp1_[2],_tmp1_[3])
    tmp_[0] = _tmp2_[0]
    tmp_[1] = RotateLeft(_tmp2_[1],1)
    tmp_[2] = RotateLeft(_tmp2_[2],12)
    tmp_[3] = RotateLeft(_tmp2_[3],13)
  cipher_[0] = tmp_[0] ^ key_[25][0]
  cipher_[1] = tmp_[1] ^ key_[25][1]
  cipher_[2] = tmp_[2] ^ key_[25][2]
  cipher_[3] = tmp_[3] ^ key_[25][3]
